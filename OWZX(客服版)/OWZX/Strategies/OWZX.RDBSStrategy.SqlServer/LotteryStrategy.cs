    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable LastLottery(string type)
        {
            lock (lkstate)
            {
                ...
                if exists(select 1 from owzx_lotteryrecord where type=10 and 
                (DATEDIFF(second,getdate(),opentime) between 0 and 300) )
                begin
                select top 1 @expect=expect, 
                @totalsec= CONVERT(VARCHAR(10),DATEDIFF(SECOND,getdate(),opentime))
                from owzx_lotteryrecord where type=10   
                ...
                if exists(select 1 from owzx_lotteryrecord where type=11 and 
                (DATEDIFF(second,getdate(),opentime) between 0 and 210))
                begin
                select top 1 @expect=expect, 
                @totalsec= CONVERT(VARCHAR(10),DATEDIFF(SECOND,getdate(),opentime))
                from owzx_lotteryrecord where type=11 and 
				(DATEDIFF(second,getdate(),opentime) between 0 and 210)
                order by lotteryid
...
", condition);
        ...
        /// <param name="rate"></param>
        /// <returns></returns>
        public string AddUserBackRule(MD_UserBackRate rate)
        {
            DbParameter[] parms = {
                                    GenerateInParam("@backfee", SqlDbType.Decimal, 18, rate.BackFee),
                                    GenerateInParam("@minfee", SqlDbType.Decimal, 18 ,rate.MinFee),
                                    GenerateInParam("@maxfee", SqlDbType.Decimal, 18, rate.MaxFee),
                                    GenerateInParam("@name", SqlDbType.VarChar, 50, rate.Name)
                                    };
            string commandText = string.Format(@"
begin try
begin tran t1

INSERT INTO [owzx_userbackrule]([backfee]
           ,[minfee]
           ,[maxfee]
           ,[Name],[addtime])
VALUES(@backfee,@minfee,@maxfee,@name,getdate())
...
");
        ...
        /// <param name="rate"></param>
        /// <returns></returns>
        public string UpdateUserBackRule(MD_UserBackRate rate)
        {
            DbParameter[] parms = {
                                      GenerateInParam("@autoid", SqlDbType.Int, 4, rate.AutoID),
                                    GenerateInParam("@backfee", SqlDbType.Decimal, 18, rate.BackFee),
                                    GenerateInParam("@minfee", SqlDbType.Decimal, 18 ,rate.MinFee),
                                    GenerateInParam("@maxfee", SqlDbType.Decimal, 18, rate.MaxFee),
                                    GenerateInParam("@name", SqlDbType.VarChar, 50, rate.Name)
            ...
            begin try
            begin tran t1
            if exists(select 1 from owzx_userbackrule where autoid=@autoid)
            begin
            UPDATE owzx_userbackrule
               SET backfee = @backfee,MinFee=@minfee,MaxFee=@maxfee ,name=@Name
            where autoid=@autoid
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public string DeleteUserBackRule(string id)
        {
            string commandText = string.Format(@"
            begin try
            if exists(select 1 from owzx_userbackrule where autoid in ({0}))
            begin
            delete from owzx_userbackrule where autoid in ({0})
            select '删除成功' state
            end
            else
            begin
        ...
        /// <param name="condition">没有where 主表 a  房间类型表 b </param>
        /// <returns></returns>
        public DataTable GetUserBackRuleList(int pageNumber, int pageSize, string condition = "")
        {
            DbParameter[] parms = {
                                      GenerateInParam("@pagesize", SqlDbType.Int, 4, pageSize),
                                      GenerateInParam("@pageindex", SqlDbType.Int, 4, pageNumber)
