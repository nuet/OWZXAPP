...
namespace OWZX.Web.controllers
{
    ...
    public class APIController : BaseWebController
    {
        ...
        public ActionResult IsTimeOut()
        {
            ...
            if (baseinfo != null)
            {
                ...
                if (DateTime.Compare(DateTime.Parse(baseinfo.Account), DateTime.Now) > 0)
                {
                    ...
                    {
                        return APIResult("error", "试用期已结束，若要继续使用软件，请联系服务人员！");
                    }
                    else
                    {
                        return APIResult("success", "正常使用");
                    }
                }
                else
                {
        ...
        public ActionResult SysTimeChange()
        {
            ...
            try
            {
                ...
                else if (type == 2)
                {
                    ...
                    {
                        base_info = new BaseInfo { Key = "试用", Name = "有效期", Account = time };
        ...
        public ActionResult GetNews()
        {
            try
            {
                ...
                if (addres != null && !addres.Columns.Contains("status"))
                {
                    ...
                    return APIResult("success", data.Replace("[", "").Replace("]", ""), true);
                }
                else
                    return APIResult("success", "暂无通知公告");
            }
        ...
        public ActionResult Message()
        {
            try
            {
                ...
                if (list.Count > 0)
                {
                    list.ForEach((x) =>
                    {
                        x.Body = BSPConfig.ShopConfig.SiteUrl + "/home/message/" + x.Msgid.ToString();
                    });
                    JsonSerializerSettings jsetting = new JsonSerializerSettings();
                    jsetting.DateFormatString = "yyyy-MM-dd HH:mm:ss";
                    jsetting.ContractResolver = new JsonLimitOutPut(new string[] { "Title", "Time", "Body" }, true);
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult DummyUser()
        {
            List<MD_Dummy> dt = NewUser.GetDummyUser(1,-1);
            JsonSerializerSettings jsetting = new JsonSerializerSettings();
            jsetting.ContractResolver = new JsonLimitOutPut(new string[] { "Username", "Nickname" }, true);
            string data = JsonConvert.SerializeObject(dt, jsetting).ToLower();
            return AjaxResult("success", data,true);
        }
        #endregion
    }
}
