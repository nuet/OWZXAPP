    ...
    public class AppAccountController : BaseWebController
    {
        ...
        public ActionResult Register()
        {
            try
            {
                ...
                int invitecode = -1;
                if (parmas.AllKeys.Contains("invitecode") && parmas["invitecode"]!="")
        ...
        public ActionResult UpdateUserInfo()
        {
            try
            {
                ...
                if (parmas["nickname"] == "8001" || parmas["nickname"] == "8002")
                {
                    return APIResult("error", "昵称已存在");
                }

                string udres = NewUser.UpdateUserInfo(account, parmas["nickname"], parmas["signname"]);
        ...
        public ActionResult UserInfo()
        {
            try
            {
                ...
                string vip = BSPConfig.ShopConfig.SiteUrl + "/images/img_";
                switch (user.UserRid-6)
                { 
                    case 1:
                        vip += (user.UserRid-6).ToString() + ".png";
                        break;
                    case 2: vip += (user.UserRid - 6).ToString() + ".png";
                        break;
                    case 3: vip += (user.UserRid - 6).ToString() + ".png";
                        break;
                    case 4: vip += (user.UserRid - 6).ToString() + ".png";
                        break;
                    case 5: vip += (user.UserRid - 6).ToString() + ".png";
                        break;
                    case 6: vip += (user.UserRid - 6).ToString() + ".png";
                        break;
                }
                string data = "{\"Account\":\"" + account + "\",\"userid\":\"" + user.UserId + "\",\"NickName\":\"" + user.NickName + "\",\"SignName\":\"" + userdetails.SignName + "\",\"Avatar\":\"" + user.Avatar + "\",\"dengji\":\"" + vip + "\"}";
